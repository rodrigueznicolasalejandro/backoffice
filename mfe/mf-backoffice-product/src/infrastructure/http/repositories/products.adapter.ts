...
export class HttpProductsRepository implements ProductsRepository {
    ...
    async patchProduct(id: string, product: Partial<Product>) {
        const {data} = await httpClient.patch<{ data: { product: Product } }>(`/products/${id}`, product);
        return data.data.product;
    }
    ...
    async getFilterValues() {
        const {data} = await httpClient.get<{ data: any }>('/products/filters/values');
        return data.data;
    }
    
    async getProductSchema() {
        const {data} = await httpClient.get<{ data: any }>('/products/schema');
        return data.data;
    }
}
